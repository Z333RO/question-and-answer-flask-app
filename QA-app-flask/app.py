...
def register():
    ...
    if request.method == 'POST':
        ...
        # You can edit the error message on the if block below to display a custom message on the registration page if a user exists.
        if existing_user:
...
def login():
    ...
    if request.method == 'POST':
        ...
        # Edit the error variable to change the error message displayed on the login portal.
        if user_result:
            ...
            if check_password_hash(user_result['password'], password):
                ...
                error = 'Incorrect credentials for this user.'
        else:
...
def users():
    ...
    # This if block protects this route from access by unauthenticated users. If this is removed, then there is potential for broken access control or directory fuzzing that can lead to access of sensitive data/pages. The block below just redirects users to the login page.
    if not user:
        return redirect(url_for('login'))
    # This next if block checks the user role to prevent low priv users from accessing routes from higher priv users like expert/admin. This code block redirects unauthorized users back to the home page.
    if user['admin'] == 0:
        return redirect(url_for('index'))
